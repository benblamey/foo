...
def cp_features(image_file_list):
    image_file_list = list(sorted(image_file_list))
    #print(pathlib.Path('.').absolute())
    #print(file_list)
    file_uris = [file.as_uri() for file in image_file_list]
    ...
    if len(file_uris) > 0:
        ...
        for i, file_uri in enumerate(file_uris):
            ...
            output_csv_paths.append(os.path.join(output_dir, "Image.csv"))
        ...
        sim = _read_data(output_csv_paths)
...
def _read_data(output_pandas):
    ...
    for i in range(0,8):
        ...
        # TODO: this offset checking is fragile wrt the input image file list
        res.append(cosine_similarity([cp_df_sim.iloc[i, :-1]],
                                     [cp_df_sim.iloc[i + 8, :-1]])[0][0])
        # else:
